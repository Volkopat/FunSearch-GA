    ...
    for individual in range(ga_config.population_size):
        ...
        while not valid_algorithm and retries < max_retries:
            try:
                ...
                full_prompt = prompt_manager.get_repeat_prompt() + parent_prompt if not valid_algorithm else parent_prompt
        ...
        if valid_algorithm:
            ...
            print(f"Failed to generate a valid parent after {max_retries} attempts.")

    selected_parents = tournament_selection(parents) + apply_elitism(parents)
